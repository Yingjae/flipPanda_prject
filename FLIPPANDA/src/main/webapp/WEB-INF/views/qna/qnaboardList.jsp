			...
			<tbody>
				<c:forEach var="qnaboardList" items="${qnaboardList }">
				   <tr style="text-align:center;"/>
						...
						<td>
						<c:if test="${qnaboardList.qnaSecret == false }">
						<c:out value="${qnaboardList.qnaTitle }">[${qnaboardList.qnaTitle }]</c:out>
						  <c:choose>
						     <c:when test="${qnaboardList.qnaWriter eq UserVO.userId }">
						      <a href="/qna/qnaboardDetail/${qnaboardList.qnaNum }?pageNum=${qnapageMaker.qnacri.pageNum}"><c:out value="${qnaboardList.qnaTitle }"/></a>
						    </c:when>
